...
SELECT U.USER_ID,U.USERNAME,P.POST_ID,C.COMMENT_TEXT
FROM USERS U
JOIN POSTS P
ON U.USER_ID=P.USER_ID
JOIN COMMENTS C
...
SELECT U.USER_ID,U.USERNAME,COUNT(P.POST_ID) AS MOSTPOSTS
FROM USERS U
JOIN POSTS P
ON U.USER_ID = P.USER_ID
GROUP BY U.USER_ID,U.USERNAME
ORDER BY MOSTPOSTS DESC LIMIT 3

---4.Retrieve posts that have more likes than the average number of likes per post.

SELECT P.POST_ID,P.CONTENT,AVG(LIKE_ID) AS AVGOFLIKES_POST
FROM POSTS P
JOIN LIKES L
ON P.POST_ID=L.POST_ID
GROUP BY P.POST_ID,P.CONTENT
ORDER BY AVGOFLIKES_POST DESC
...
SELECT U.USER_ID,U.USERNAME FROM FRIENDSHIPS F
JOIN USERS U
ON (U.USER_ID=F.USER_ID2 AND F.USER_ID1=3)
OR (U.USER_ID=F.USER_ID1 AND F.USER_ID2=3)
...
SELECT DISTINCT P.Post_ID,
       P.Content,
       U.Username AS PostOwner,
       F.Username AS FriendWhoLiked
FROM Posts P
...
WHERE EXISTS (
    ...
    WHERE (FR.User_ID1 = @givenUserId AND FR.User_ID2 = L.User_ID)
       OR (FR.User_ID2 = @givenUserId AND FR.User_ID1 = L.User_ID)
);
...
JOIN Users F ON L.User_ID = F.User_ID
WHERE L.User_ID IN (
    SELECT IF(User_ID1 = @givenuserid, User_ID2, User_ID1)
    FROM Friendships
    WHERE User_ID1 = @givenuserid OR User_ID2 = @givenuserid
);

DELIMITER $$

CREATE PROCEDURE GetUserActivity(IN p_userId INT)
BEGIN
    -- 1. User Posts
    ...
    SELECT P.Post_ID, P.Content, U.Username AS PostOwner
    FROM Likes L
    ...
    -- 3. Friends of User
    SELECT CASE 
              WHEN F.User_ID1 = p_userId THEN F.User_ID2 
              ELSE F.User_ID1 
           END AS Friend_ID,
    ...
    JOIN Users U 
         ON U.User_ID = CASE 
                            ...
                            ELSE F.User_ID1 
                         END
    WHERE F.User_ID1 = p_userId OR F.User_ID2 = p_userId;
END$$

DELIMITER ;

CALL GetUserActivity(8);

SELECT U.User_ID,
       U.Username,
       COUNT(DISTINCT L.Like_ID) + COUNT(DISTINCT C.Comment_ID) AS InfluenceScore
FROM Users U
JOIN Posts P 
     ON U.User_ID = P.User_ID
LEFT JOIN Likes L 
